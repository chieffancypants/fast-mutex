...
var debug = require('debug')('FastMutex');
 ...
 */
var resolveWithStats = function resolveWithStats(resolve, stats) {
...
var FastMutex = function () {
  function FastMutex() {
    var _ref = arguments.length <= 0 || arguments[0] === undefined ? {} : arguments[0];

    var _ref$clientId = _ref.clientId;
    var clientId = _ref$clientId === undefined ? randomId() : _ref$clientId;
    var _ref$xPrefix = _ref.xPrefix;
    var xPrefix = _ref$xPrefix === undefined ? '_MUTEX_LOCK_X_' : _ref$xPrefix;
    var _ref$yPrefix = _ref.yPrefix;
    var yPrefix = _ref$yPrefix === undefined ? '_MUTEX_LOCK_Y_' : _ref$yPrefix;
    var _ref$timeout = _ref.timeout;
    var timeout = _ref$timeout === undefined ? 5000 : _ref$timeout;
    var localStorage = _ref.localStorage;

    _classCallCheck(this, FastMutex);
  ...
  _createClass(FastMutex, [{
    ...
    value: function lock(key) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        var acquireLock = function acquireLock(key) {
          ...
          var elapsedTime = new Date().getTime() - _this.lockStats.acquireStart;
          if (elapsedTime >= _this.timeout) {
            debug('Lock on "%s" could not be acquired within %sms by FastMutex client "%s"', key, _this.timeout, _this.clientId);
            return reject(new Error('Lock could not be acquired within ' + _this.timeout + 'ms'));
          }

          _this.setItem(x, _this.clientId);

          // if y exists, another client is getting a lock, so retry in a bit
          var lsY = _this.getItem(y);
          if (lsY) {
            debug('Lock exists on Y (%s), restarting...', lsY);
            _this.lockStats.restartCount++;
            setTimeout(function () {
          ...
          // ask for inner lock
          _this.setItem(y, _this.clientId);

          // if x was changed, another client is contending for an inner lock
          var lsX = _this.getItem(x);
          if (lsX !== _this.clientId) {
            _this.lockStats.contentionCount++;
            debug('Lock contention detected. X="%s"', lsX);

            // Give enough time for critical section:
            setTimeout(function () {
              lsY = _this.getItem(y);
              if (lsY === _this.clientId) {
                // we have a lock
                debug('FastMutex client "%s" won the lock contention on "%s"', _this.clientId, key);
                resolveWithStats(resolve, _this.lockStats);
              } else {
                // we lost the lock, restart the process again
                _this.lockStats.restartCount++;
                _this.lockStats.locksLost++;
                debug('FastMutex client "%s" lost the lock contention on "%s" to another process (%s). Restarting...', _this.clientId, key, lsY);
                setTimeout(function () {
                  return acquireLock(key);
                });
              }
          ...
          // no contention:
          debug('FastMutex client "%s" acquired a lock on "%s" with no contention', _this.clientId, key);
          resolveWithStats(resolve, _this.lockStats);
        };
    ...
    value: function release(key) {
      ...
      return new Promise(function (resolve) {
        _this2.localStorage.removeItem(y);
        _this2.lockStats.lockEnd = new Date().getTime();
        _this2.lockStats.lockDuration = _this2.lockStats.lockEnd - _this2.lockStats.lockStart;
        resolve(_this2.lockStats);
        _this2.resetStats();
      });
    }
